...
function main (args) {
  ...
  const msgMetrics = [
    ...
    ['.', 'reject-spamassassin', { color: '#bc2d02', label: 'Rejected: Spamassassin' }],
    ['.', 'reject-virus', { color: '#a80000', label: 'Rejected: Virus' }],
    ['.', 'reject-nomx', { color: '#bc2d02', label: 'Rejected: No MX' }],
  ...
  const logs = [
    {
      query: "SOURCE '${SpamLogGroup}' | SOURCE '${SpamassassinLogGroup}' | fields @timestamp as timestamp,reason,from,recipient | filter _logLevel='info' and msg='reject' | sort @timestamp desc | limit 1000", // eslint-disable-line no-template-curly-in-string
      title: 'Rejected'
    },
    {
